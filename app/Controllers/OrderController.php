...
use Braintree_Transaction as BT_Transaction;
...
class OrderController
{
	...
	public function create(Request $request, Response $response, Customer $customer, Address $address) {
		...
		if ($validation->fails() OR !$request->getParam('payment_method_nonce')) {
		...
		$order = $customer->orders()->create([
			'hash' => $hash,
			'paid' => false,
			'total' => $this->basket->subTotal() + 5.5,
		...
		$result = BT_Transaction::sale([
			'amount' => $this->basket->subTotal() + 5.5,
